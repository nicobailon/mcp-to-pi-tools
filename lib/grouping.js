...
import { isValidToolName } from "./schema-converter.js";

const execAsync = promisify(exec);

/**
 * Generate the grouping prompt for Pi (extension format)
...
function generateGroupingPrompt(serverName, tools) {
  ...
  return `You are grouping MCP tools into pi extension tool registrations.
...
Group these tools into logical pi extension tools. Guidelines:
- Group related actions (e.g., all mouse interactions under ${serverName}_interact)
- Create dedicated tools for high-frequency operations (e.g., screenshot gets its own tool)
- Keep groups cohesive (max 5-6 MCP tools per group, fewer for complex tools)
- Tool names MUST be snake_case (e.g., ${serverName}_navigate, chrome_interact)
- Maximum 20 grouped tools total
- Single-tool groups are fine for important/complex tools

For multi-tool groups:
- The MCP tool names become "action" enum values
- Parameters from all tools are merged (common params stay required, tool-specific become optional)

Output ONLY valid JSON (no markdown, no explanation):
{
  "groups": [
    {
      "toolName": "${serverName}_example",
      "label": "Example Tool",
      "description": "One-line description of what this grouped tool does",
      "mcpTools": ["mcp_tool_1", "mcp_tool_2"],
...
/**
 * Generate the grouping prompt for CLI wrappers (legacy format)
 * @param {string} serverName - MCP server name
 * @param {Array} tools - Array of tool objects with name, description, inputSchema
 * @returns {string} - Prompt for Pi
 */
function generateCliGroupingPrompt(serverName, tools) {
  const toolList = tools
    .map((t) => {
      const params = t.inputSchema?.properties
        ? Object.entries(t.inputSchema.properties)
...
/**
 * Validate the grouping response for extension format
 * @param {object} response - Parsed response
 * @param {Array} tools - Original tools array
 * @returns {object} - Validated response
 */
function validateExtensionGrouping(response, tools) {
  ...
  for (const group of response.groups) {
    ...
    if (!group.toolName || !group.mcpTools || !Array.isArray(group.mcpTools)) {
      throw new Error(`Invalid group: missing toolName or mcpTools - ${JSON.stringify(group)}`);
    }

    // Validate tool name format (snake_case)
    if (!isValidToolName(group.toolName)) {
      throw new Error(
        `Invalid tool name: "${group.toolName}" - must be snake_case (lowercase, underscores, start with letter)`
      );
    }

    // Validate label exists
    if (!group.label) {
      // Auto-generate label from toolName
      group.label = group.toolName
        .split("_")
    ...
    if (!group.description) {
      throw new Error(`Missing description for group: ${group.toolName}`);
    }

    // Verify all referenced tools exist
    for (const toolName of group.mcpTools) {
      if (!toolNames.has(toolName)) {
        throw new Error(`Unknown MCP tool referenced: ${toolName} in group ${group.toolName}`);
      }
  ...
  if (unusedTools.length > 0) {
    ...
    if (process.env.MCP2CLI_ALLOW_UNUSED === "true" || process.env.MCP2EXT_ALLOW_UNUSED === "true") {
      console.warn(`      Warning: ${message}`);
    } else {
      throw new Error(
        `${message}\n      Set MCP2EXT_ALLOW_UNUSED=true to allow unused tools, or ensure all tools are grouped.`
      );
    }
  }
...
/**
 * Validate the grouping response for CLI format (legacy)
 * @param {object} response - Parsed response
 * @param {Array} tools - Original tools array
 * @returns {object} - Validated response
 */
function validateCliGrouping(response, tools) {
  if (!response.groups || !Array.isArray(response.groups)) {
    throw new Error("Invalid grouping response: missing groups array");
  }
 ...
 * @param {string} agentType - "pi", "claude", or "codex"
...
function buildAgentCommand(tempFile, agentType) {
  if (agentType === "pi") {
    ...
    return `cat "${tempFile}" | codex exec - --full-auto`;
  }
  throw new Error(`Unknown agent type: ${agentType}`);
}

/**
 * Group tools using AI agent's non-interactive mode (extension format)
 * @param {string} serverName - MCP server name
 * @param {Array} tools - Array of tool objects
 * @param {object} options - options
 * @param {boolean} options.quiet - suppress progress output
 * @param {string} options.agentType - "pi", "claude", or "codex"
 * @returns {Promise<Array>} - Array of group objects
 */
export async function groupToolsForExtension(serverName, tools, options = {}) {
  const { quiet = false, agentType = "pi" } = options;

  const prompt = generateGroupingPrompt(serverName, tools);
  const tempFile = join(tmpdir(), `mcp2ext-grouping-${Date.now()}.md`);
  ...
  try {
    ...
    const validated = validateExtensionGrouping(response, tools);
...
/**
 * Group tools using AI agent's non-interactive mode (CLI format - legacy)
 * @param {string} serverName - MCP server name
 * @param {Array} tools - Array of tool objects
 * @param {object} options - options
 * @param {boolean} options.quiet - suppress progress output
...
export async function groupTools(serverName, tools, options = {}) {
  ...
  const prompt = generateCliGroupingPrompt(serverName, tools);
  ...
  try {
    ...
    const response = parseGroupingResponse(stdout);
    const validated = validateCliGrouping(response, tools);
...
/**
 * Sanitize a string to be a valid snake_case identifier component
 * @param {string} str - String to sanitize
 * @returns {string} - Sanitized string (lowercase, alphanumeric and underscores only)
 */
...
/**
 * Fallback grouping when no AI agent (Pi/Claude) is available (extension format)
 * Creates one tool per MCP tool (simple but works)
 * @param {string} serverName - MCP server name
 * @param {Array} tools - Array of tool objects
 * @returns {Array} - Array of group objects
 */

export function fallbackGroupingForExtension(serverName, tools) {
  // Convert server name to valid snake_case
  const safeServerName = sanitizeToSnakeCase(serverName) || "mcp";

  return tools.map((tool) => {
    // Sanitize tool name as well
    const safeMcpToolName = sanitizeToSnakeCase(tool.name) || "tool";
    const toolName = `${safeServerName}_${safeMcpToolName}`;
...
/**
 * Fallback grouping when no AI agent (Pi/Claude) is available (CLI format)
