...
function validateGrouping(response, tools) {
  ...
  // Warn about unused tools (don't fail)
...
export async function groupTools(serverName, tools, options = {}) {
  ...
  // Call Pi in non-interactive mode
  // Using --no-session to avoid persisting this ephemeral session
  // Write prompt to temp file and use @file syntax
  ...
  try {
    ...
    if (error.stderr && error.stderr.includes("pi")) {
...
/**
 * Fallback grouping when Pi is not available
