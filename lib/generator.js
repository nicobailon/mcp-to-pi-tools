...
import { escapeTemplateLiteral, escapeDoubleQuotedString } from "./runner.js";
 ...
 * @param {string} mcpCommand - npx command for MCP server (null for HTTP)
 ...
 */
function generateWrapperPrompt(group, tools, serverName, mcpCommand, httpOptions = {}) {
...
Parameters: ${propsStr}`;
  ...
  const connectionInfo = isHttpMode
    ? `HTTP URL: ${httpOptions.httpUrl}`
    : `MCP Server Command: ${mcpCommand}`;

  // Build mcporter call example based on transport
  // Escape URLs and commands for safe embedding in template literals
  let mcporterCallExample;
  if (isHttpMode) {
    const allowHttpFlag = httpOptions.allowHttp ? "--allow-http " : "";
    const escapedHttpUrl = escapeTemplateLiteral(httpOptions.httpUrl);
    mcporterCallExample = `function callMcp(tool, params = {}) {
  const paramStr = Object.entries(params)
    .map(([k, v]) => \`\${k}:\${JSON.stringify(v)}\`)
    .join(" ");
  const cmd = \`npx mcporter call ${allowHttpFlag}--http-url "${escapedHttpUrl}" ${serverName}.\${tool} \${paramStr}\`;
  try {
    ...
    const escapedMcpCommand = escapeTemplateLiteral(mcpCommand);
    mcporterCallExample = `function callMcp(tool, params = {}) {
  const paramStr = Object.entries(params)
    .map(([k, v]) => \`\${k}:\${JSON.stringify(v)}\`)
    .join(" ");
  const cmd = \`npx mcporter call --stdio "${escapedMcpCommand}" ${serverName}.\${tool} \${paramStr}\`;
  try {
    return execSync(cmd, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
  } catch (error) {
    throw new Error(error.stderr || error.message);
  ...
  // Build mcporter call instruction (plain text for prompt, no escaping needed)
  const mcporterCallInstruction = isHttpMode
    ? `npx mcporter call ${httpOptions.allowHttp ? "--allow-http " : ""}--http-url "${httpOptions.httpUrl}" ${serverName}.<tool_name> param:value`
    : `npx mcporter call --stdio "${mcpCommand}" ${serverName}.<tool_name> param:value`;
...
5. Call MCP via execSync: ${mcporterCallInstruction}
 ...
 * @param {string} options.httpUrl - HTTP endpoint URL (optional)
 * @param {boolean} options.allowHttp - Whether --allow-http is needed (optional)
...
export async function generateWrapper(group, tools, serverName, mcpCommand, options = {}) {
  const { quiet = false, agentType = "pi", httpUrl, allowHttp } = options;

  const httpOptions = httpUrl ? { httpUrl, allowHttp } : {};
  const prompt = generateWrapperPrompt(group, tools, serverName, mcpCommand, httpOptions);
...
3. ## Usage section showing symlink invocation (preferred) and full path (alternative)
4. ## How to Invoke section explaining symlinks are shortest, full path works, node prefix is wrong
...
export function generateBasicReadme(name, groups) {
  ...
  const firstToolNoExt = firstTool.replace(/\.js$/, "");
...
\`\`\`bash
# Via symlink (shortest, preferred)
${firstToolNoExt} --help

# Via full path (also works)
~/agent-tools/${name}/${firstTool} --help
\`\`\`

## How to Invoke

**PREFERRED** - Symlink (shortest):
\`\`\`bash
${firstToolNoExt} --help
\`\`\`
...
**INCORRECT** - Don't use node prefix:
\`\`\`bash
node ~/agent-tools/${name}/${firstTool}  # Wrong
\`\`\`
