...
import { escapeTemplateLiteral } from "./runner.js";
 ...
 * @param {string} mcpCommand - npx command for MCP server (STDIO mode)
...
Parameters: ${propsStr}`;
  ...
  // Determine transport mode and build mcporter call syntax
  const isHttpMode = httpOptions?.httpUrl;
  let transportInfo;
  let mcporterCallSyntax;
  let exampleHelper;

  if (isHttpMode) {
    const allowHttpFlag = httpOptions.allowHttp ? " --allow-http" : "";
    const escapedUrl = escapeTemplateLiteral(httpOptions.httpUrl);
    const escapedAllowHttp = escapeTemplateLiteral(allowHttpFlag);
    transportInfo = `MCP HTTP Endpoint: ${httpOptions.httpUrl}${httpOptions.allowHttp ? " (--allow-http)" : ""}`;
    mcporterCallSyntax = `npx mcporter call${allowHttpFlag} --http-url "${httpOptions.httpUrl}" ${serverName}.<tool_name> param:value`;
    exampleHelper = `function callMcp(tool, params = {}) {
  const paramStr = Object.entries(params)
    .map(([k, v]) => \`\${k}:\${JSON.stringify(v)}\`)
    .join(" ");
  const cmd = \`npx mcporter call${escapedAllowHttp} --http-url "${escapedUrl}" ${serverName}.\${tool} \${paramStr}\`;
  try {
    ...
    const escapedCmd = escapeTemplateLiteral(mcpCommand);
    transportInfo = `MCP Server Command: ${mcpCommand}`;
    mcporterCallSyntax = `npx mcporter call --stdio "${mcpCommand}" ${serverName}.<tool_name> param:value`;
    exampleHelper = `function callMcp(tool, params = {}) {
  const paramStr = Object.entries(params)
    .map(([k, v]) => \`\${k}:\${JSON.stringify(v)}\`)
    .join(" ");
  const cmd = \`npx mcporter call --stdio "${escapedCmd}" ${serverName}.\${tool} \${paramStr}\`;
  try {
    return execSync(cmd, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] });
  } catch (error) {
    throw new Error(error.stderr || error.message);
...
5. Call MCP via execSync: ${mcporterCallSyntax}
 ...
 * @param {object} [options.httpOptions] - HTTP transport options
 * @param {string} [options.httpOptions.httpUrl] - HTTP URL for MCP endpoint
 * @param {boolean} [options.httpOptions.allowHttp] - Allow non-localhost HTTP
...
export async function generateWrapper(group, tools, serverName, mcpCommand, options = {}) {
  const { quiet = false, agentType = "pi", httpOptions } = options;
