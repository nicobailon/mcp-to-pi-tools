...
export async function generateWrapper(group, tools, serverName, mcpCommand, options = {}) {
  ...
  try {
    ...
    const { stdout } = await execAsync(`pi -p --no-session --tools "" @"${tempFile}"`, {
      encoding: "utf-8",
 ...
 */
export function generateInstallScript(name) {
  return `#!/bin/bash
...
3. ## Setup section with install instructions:
   cd ~/agent-tools/${name}
   ./install.sh
4. ## How to Invoke section explaining CORRECT (just script name) vs INCORRECT (node prefix, ./ prefix)
 ...
 * @returns {string}
 */
export function generateAgentsEntry(name, groups) {
  const toolTable = groups
    .map((g) => `| \`${g.filename}\` | ${g.description} |`)
    .join("\n");
