  ...
  try {
    ...
    if (heading) {
      ...
      if (existing) {
        ...
        updateRegistry(name, targetPath, heading);
...
/**
 * Update registry with registration info
 * @param {string} name - Tool directory name
 * @param {string} path - Path where registered
 * @param {string} heading - Heading used
 */
function updateRegistry(name, path, heading) {
